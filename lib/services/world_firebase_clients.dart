...
typedef WorldClientBuilder<T> = T Function(WorldDescriptor descriptor);
...
final class WorldClientCache<T> {
  ...
  T forWorld(WorldDescriptor descriptor) {
    final worldId = WorldId(descriptor.worldId);
    return _clients.putIfAbsent(worldId, () => _builder(descriptor));
  }
}
...
final class WorldFirebaseClients {
  ...
  final WorldDescriptor descriptor;
...
final class WorldFirebaseClientCache {
  WorldFirebaseClientCache({required FirebaseApp app, String? emulatorHost})
    : _app = app,
      ...
      _clients = WorldClientCache<WorldFirebaseClients>(
        (descriptor) => _createClients(app, descriptor, emulatorHost),
      );
  ...
  WorldFirebaseClients forWorld(WorldDescriptor descriptor) {
    return _clients.forWorld(descriptor);
  }
  ...
  static WorldFirebaseClients _createClients(
    ...
    final functions = FirebaseFunctions.instanceFor(
      app: app,
      region: descriptor.functionsRegion,
    );
    ...
    if (firestore.databaseId != descriptor.databaseId) {
      throw StateError(
        'Firestore route mismatch for ${descriptor.worldId}: '
        '${firestore.databaseId}',
      );
    }
    if (storage.bucket != descriptor.bucketName) {
      throw StateError(
        'Storage route mismatch for ${descriptor.worldId}: ${storage.bucket}',
      );
    }
    ...
    return WorldFirebaseClients(
      worldId: WorldId(descriptor.worldId),
