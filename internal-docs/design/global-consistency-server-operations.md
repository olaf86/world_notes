...
| `setMessageLike` | User/note message-like state and message aggregate | `W` | `L1` | Exact in the message's world | One user serializes their likes within one note; a highly popular message can still hotspot its aggregate document. Shard the message count only if observed. |
